...
void index_make_entry(char *buf, size_t buflen, struct Menu *menu, int line)
{
  ...
  mutt_make_string_flags(buf, buflen, menu->win_index->state.cols,
                         NONULL(C_IndexFormat), Context->mailbox,
                         Context->msg_in_pager, e, flags);
}
    ...
    switch (op)
    {
        ...
        if (Context->mailbox->type == MUTT_NNTP)
        {
          ...
          else
          {
            ...
            if (rc == 0)
            {
              ...
              mutt_sort_headers(Context->mailbox, Context->threads, false, &Context->vsize);
              menu->current = e->vnum;
              menu->redraw = REDRAW_FULL;
            }
            else if (rc > 0)
      ...
      case OP_RECONSTRUCT_THREAD:
      {
        ...
        if (Context->mailbox->msg_count > oldmsgcount)
        {
          ...
          if (rc < 0)
            Context->mailbox->verbose = false;
          mutt_sort_headers(Context->mailbox, Context->threads, (op == OP_RECONSTRUCT_THREAD), &Context->vsize);
          Context->mailbox->verbose = verbose;
